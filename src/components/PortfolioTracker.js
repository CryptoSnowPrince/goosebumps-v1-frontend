...
const PortfolioTracker = () => {
    ...
    const fetchLiveInfo = async (tokens) => {
        ...
        // }
       // console.log(addresses)//wrong
       const pair=0;

        const token = await PairHelper.getTokenInfo(pair, addresses, network);
        console.log(token)

        // const infos = await PairHelper.getTokenInfos(linq.from(tokens).select(x => x.pair).toArray(), network, addresses);
        // const query = linq.from(infos.infos); //doesnt work
