...
async function getInfo(address, network, pairAddress) {
    const pairs = await Requester.getAsync("api/Charts/GetPairs", { address: address, network: network.Name });//no clue why it exists
    if (pairAddress == null) {
        var pair = pairs[0];
    }
    else {
            ...
            <div className="container-fluid">
                <div className="row">
                    ...
                    <div className="col-lg-9 mt-4 mt-lg-0">
                        <div className="row">
                            <div ref={exchangeContainerRef} className={"mb-4 mb-xl-0 order-1 col-xl-7"}>
                                <div className='row h-100 m-0'>
                                    <div className='col p-0'>
                                        <Chart title={info.title} pair={info.pair} network={network} />
                                    </div>
                                    <div className='d-none d-xl-block col-auto hider fa fa-arrow-right' ref={hideExchangeRef} onClick={onHideExchange}></div>
                                </div>
                            </div>
                            <div className="col-xl-5 order-3 order-xl-2 mt-4 mt-xl-0 ps-xl-0" ref={exchangeRef}>
                                <Exchange network={network} fromSymbol={info.pair.sellCurrency.symbol} fromAddress={info.pair.sellCurrency.address} toSymbol={info.pair.buyCurrency.symbol} toAddress={info.pair.buyCurrency.address} />
